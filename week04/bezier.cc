...
void winReshapeFcn (GLint newWidth, GLint newHeight)
{
   ...
   glViewport (0, 0, newHeight / 2, newHeight / 2);
...
{
	if(btn==GLUT_WHEEL_DOWN && GLUT_ACTIVE_CTRL) glScalef(2.0, 2.0, 0.0); ; //ZOOM IN 

	if(btn==GLUT_WHEEL_UP && GLUT_ACTIVE_CTRL ) glScalef(0.5, 0.5, 0.0);; //ZOOM OUT

	if(btn==GLUT_MIDDLE_BUTTON && state==GLUT_DOWN) size=20;
